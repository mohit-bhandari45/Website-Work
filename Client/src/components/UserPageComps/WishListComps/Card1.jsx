...
const Card1 = ({ imageId,imageUrl, discount, title, mainPrice, prevPrice, rating, reviews,refreshData }) => {
    const [visible, setvisible] = useState(false)
    const navigate=useNavigate()

    return (
        <div className='h-full w-[21vw] rounded-lg relative flex flex-col justify-center items-center font-[Helvetica]'>
            <div onClick={()=>navigate(`/productdetails/${imageId}`)} onMouseOver={() => { setvisible(true) }} onMouseLeave={() => { setvisible(false) }} className="card h-[50vh] w-full bg-[#F5F5F5] relative flex justify-center items-center cursor-pointer hover:scale-95 transition-all ease-in-out duration-500">
                ...
                <div className="icons w-full flex justify-between items-start h-full p-3">
                    ...
                    <div className="mainicons flex flex-col gap-2 justify-center items-center">
                        <div className="love p-2 bg-white rounded-full"><img src={Delete} alt="" /></div>
                    </div>
                </div>
