    ...
    return (
        <div className='h-[130vh] w-full font-[Helvetica] flex flex-col justify-center items-center'>
            ...
            <div className='w-[90%] h-[90vh] flex justify-start items-center gap-8'>
                <div onClick={()=>navigate(`/productdetails/${recentProducts[0]._id}`)} className="left cursor-pointer h-full w-[45%] bg-black box-border px-5 py-5 rounded-lg text-white gap-2 object-cover flex flex-col relative justify-end items-start">
                    <img onClick={()=>navigate(`/productdetails/${recentProducts[0]._id}`)} src={`${getImage}/${recentProducts[0].imageUrl}`} className='w-[80%] h-[80%] cursor-pointer absolute bottom-0 right-5' alt="" />
                ...
                <div className="right h-full w-[55%] box-border flex flex-col gap-8">
                    <div onClick={()=>navigate(`/productdetails/${recentProducts[1]._id}`)} className="top h-[50%] cursor-pointer w-full bg-black rounded-md px-6 justify-between flex items-end">
                        ...
                        <div onClick={()=>navigate(`/productdetails/${recentProducts[1]._id}`)} className="image">
                            <img src={`${getImage}/${recentProducts[1].imageUrl}`} alt="" />
                        </div>
                    </div>
                    <div className='bottom h-[50%] w-full flex justify-center items-center gap-10'>
                        <div onClick={()=>navigate(`/productdetails/${recentProducts[2]._id}`)} className="first w-1/2 cursor-pointer bg-black h-full rounded-md relative flex flex-col justify-end py-4 px-4">
                            <img onClick={()=>navigate(`/productdetails/${recentProducts[2]._id}`)} className='absolute top-8 left-28 z-10 w-56 h-56' src={`${getImage}/${recentProducts[2].imageUrl}`} alt="" />
                        ...
                        <div onClick={()=>navigate(`/productdetails/${recentProducts[3]._id}`)} className="second w-1/2 cursor-pointer bg-black h-full rounded-lg flex justify-center items-center">
